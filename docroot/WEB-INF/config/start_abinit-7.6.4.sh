...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
INPUT_FILE1=`basename $1 | awk -F'_' '{print $NF}'`
INPUT_FILE3=`basename $3 | awk -F'_' '{print $NF}'`

mv `basename $1` ${INPUT_FILE1} 2>/dev/null
mv `basename $3` ${INPUT_FILE3} 2>/dev/null

file `basename ${2}` | grep -i tar >/dev/null
if [ $? -eq 0 ] ; then
	RES=0
	echo;echo "Extracting the pseudo potential file(s) from $2"
	echo "tar xvf `basename ${2}` --strip-components 1 >.files"
	tar xvf `basename ${2}` --strip-components 1 >.files
	for file in `cat .files`
	do
		echo $file
	        INPUT_FILE2_tmp=`basename ${file} 2>/dev/null | awk -F'_' '{print $NF}'`
        	echo "mv `basename ${file}` ${INPUT_FILE2_tmp}" #2>/dev/null
	        mv `basename ${file}` ${INPUT_FILE2_tmp} #2>/dev/null
	done
else
	RES=1
	INPUT_FILE2=`basename $2 | awk -F'_' '{print $NF}'`
...
if [ "X${MIDDLEWARE}" = "XgLite" ] ; then
	...
	if [ "X${RELEASE}" == "Xel5" ] ; then
 		echo "Settings preferences for el5 release"
		...
		echo "Settings preferences for el6 release"
	...
	if [ "X${DOMAIN_NAME}" == "Xwn-1.grid.arn.dz" ]; then
		echo "Settings preferences for ARN"
	...
	if [ "X${RES}" = "X0" ] ; then
		echo "PSEUDO FILE(s)  : "`cat .files`
	else
		echo "PSEUDO FILE(s)  : "${INPUT_FILE2}
	fi
	...
	if [ "X${OUTPUTS_LIST}" = "X" ] ; then
		echo;echo "[ INFO ] You haven't specify any additional output files. "
                echo "[ INFO ] Only std OUT/ERR files will be collected."
	else
	...
	if [ $? -eq 1 ] ; then
		echo "The DPM_STORAGE_PATH [ ${DPM_STORAGE_PATH} ] does already does exist on the Grid Storage Element"
	else
...
if [ -f ${ABINIT_PATH}/abinit ] && [ "X${FLAG}" = "Xtrue" ] ; then
	...
	if [ "X${URL}" != "X" ] ; then
		...
		./curl -3 -k -E ${PROXY_RFC} -X POST \
	       -d "__Replicas=https://${DPM_HOST}${DPM_STORAGE_PATH}/${VO_NAME}/ABINIT/abinit_${USERNAME}_${DATETIME}.tar.gz&FileName=abinit_${USERNAME}_${DATETIME}.tar.gz&Size=${SIZE}&Description=${DESC}&FileType=GZIP&Creator=${USERNAME}&SubmissionDate=${TIMESTAMP}" \
	       https://${METADATA_HOST}/api/ABINIT/Jobs/
	else 
		echo "Some errors occurred during the file registration. Please, check log messages"
		echo "./curl -3 -k -E ${PROXY_RFC} https://${METADATA_HOST}/api/dm/put/${VO_NAME}/abinit_${USERNAME}_${DATETIME}.tar.gz/${DPM_HOST}${DPM_STORAGE_PATH}/home/${VO_NAME}/ABINIT/"
